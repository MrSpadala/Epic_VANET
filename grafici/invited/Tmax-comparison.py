...
for city, scenarios in city_scenario.items():
    for scenario in scenarios:
        ...
        print(f"Executing on {city} {scenario}")

        for i, tmax in enumerate(tmax_vals):
            sim_config.config.Tmax = tmax
            
            sims = performSimulations(sim_config.config.nsimulations, verbose=False)
            res = computeMetrics(sims, verbose=False)
